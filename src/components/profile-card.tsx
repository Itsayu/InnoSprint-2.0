...
export function ProfileCard({ name, title, imageUrl, dataAiHint, onCardClick }: Omit<ProfileCardProps, 'description' | 'socials'> & {onCardClick?: () => void}) {
  return (
    <Card 
        className="text-center bg-card/50 backdrop-blur-lg hover:bg-card/80 transition-all duration-300 ease-in-out group border-border/50 hover:shadow-primary/20 hover:shadow-lg cursor-pointer hover:-translate-y-1"
        onClick={onCardClick}
    >
